...
| `api_core/thresholding.py` | **Stage 2.** Loads `scores.npy` from Stage 1, optionally applies Gaussian score smoothing, then applies the selected thresholding strategy (head-tail breaks, carve_fraction, or a numeric value) on the raw scores to produce a binary carving mask, and writes `mask.npy`. Can be re-run with different threshold parameters without re-computing scores. |
| `api_core/exporting.py` | **Stage 3.** Loads the binary mask from Stage 2, prunes small disconnected voxel clusters, reconstructs a triangle mesh (either cubic voxel faces or SDF-smoothed marching cubes with Taubin polishing), and writes the final `export.<format>` mesh. |
...
| `simulated_weights.py` | Simulated benefit weights (opt-in via `simulated_weights_path`). Implements the ISO 13790 Annex C hourly thermal model ("5R1C": five conductances, one lumped mass capacitance, one zone) in pure NumPy, and derives the hourly *marginal* benefit/harm of solar gain by perturbation: for each hour of the year, an extra watt of solar gain is added/removed and the change in annual heating and cooling demand is attributed to that hour (`benefit[t]` / `harm[t]`, both in [0, 1]; all 17 521 perturbed year-simulations march as one vectorized batch). **`generate_simulated_weights`**: EPW + archetype → `simulated_weights.json` artifact with provenance metadata plus a `.png` heatmap preview. **`ZoneParams.from_archetype`** builds the thermal network from ~10 physical inputs; **`expand_shoebox`** derives all areas from a width/length/height + per-facade WWR shorthand. Validated against ETH's RC_BuildingSimulator (`tests/data/oracle_5r1c_reference.json`). Design note: `design/simulated-weights.md`. |
...
└── test_simulated_weights.py  5R1C engine: oracle regression vs RC_BuildingSimulator,
│                       steady-state closure, attribution structure (seasonality,
│                       thermal-lag credit), shoebox expansion, artifact I/O
```
