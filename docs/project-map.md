...
| `usefulness.py` | Simulated benefit weights (opt-in via `usefulness_path`). Implements the ISO 13790 Annex C hourly thermal model ("5R1C": five conductances, one lumped mass capacitance, one zone) in pure NumPy, and derives hourly *marginal* solar usefulness by perturbation: for each hour of the year, an extra watt of solar gain is added/removed and the change in annual heating and cooling demand is attributed to that hour (`benefit[t]` / `harm[t]`, both in [0, 1]; all 17 521 perturbed year-simulations march as one vectorized batch). **`generate_usefulness`**: EPW + archetype → `solar_usefulness.json` artifact with provenance metadata plus a `.png` heatmap preview. **`ZoneParams.from_archetype`** builds the thermal network from ~10 physical inputs; **`expand_shoebox`** derives all areas from a width/length/height + per-facade WWR shorthand. Validated against ETH's RC_BuildingSimulator (`tests/data/oracle_5r1c_reference.json`). Design note: `design/solar-usefulness.md`. |
...
└── test_usefulness.py  5R1C engine: oracle regression vs RC_BuildingSimulator,
